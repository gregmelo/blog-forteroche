...
<div class="comments">
    ...
    if (empty($comments)) {
        ...
        foreach ($comments as $comment) {
            ...
            echo '          </div>';
            echo '      </div>';
            // Afficher le bouton si admin connecté
            if (isset($_SESSION['user'])) {
                echo    '<div class="btn-supprimer"><a href="index.php?action=deleteComment&id=' . $comment->getId() . '" ' . Utils::askConfirmation("Êtes-vous sûr de vouloir supprimer ce commentaire ?") . '>Supprimer</a></div>';
            }
            echo '  </div>';
            echo '</li>';
        }
